...
import { agruparOrgaosSetores, type OrgaoGrupo } from "@/lib/servicos";
...
export function OrgaosSetoresTab({
  ...
  const { grupos, temSetor } = useMemo(() => agruparOrgaosSetores(orgaos, cartas), [orgaos, cartas]);
  const insightConcentracao = useMemo(() => calcularInsightConcentracaoOrgaos(grupos, temSetor), [grupos, temSetor]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {insightConcentracao && (
        <StoryCard
          anchor={`${insightConcentracao.orgaoSigla} concentra ${insightConcentracao.participacaoPct.toFixed(0)}% dos serviços cadastrados, distribuídos em ${insightConcentracao.totalSetores} setores.`}
          caption={`${insightConcentracao.totalOrgaos} órgãos cadastram serviços no portal — quanto mais concentrado num só, menos distribuída fica a oferta.`}
          comoLer="Participação alta com poucos setores indica um órgão mais centralizado; participação alta com muitos setores indica um órgão grande, mas organizado internamente em várias áreas."
        />
      )}
...
function OrgaoSetorTable({ grupos, expansivel }: { grupos: OrgaoGrupo[]; expansivel: boolean }) {
  ...
  return (
    <div className="flex flex-col">
      <div
        className="flex items-center gap-2 px-3 pb-1.5 text-xs font-semibold uppercase tracking-wide"
        style={{ color: "var(--ds-color-text-muted)" }}
      >
        <span className="flex-1 min-w-0">Órgão</span>
          ...
          return (
            <li key={g.orgaoSigla}>
              <button
                ...
                className="w-full flex items-center gap-2 py-1.5 px-3 rounded text-sm text-left"
              ...
              >
                ...
                {expansivel && (
                  <span className="w-14 text-right shrink-0 text-xs" style={{ color: "var(--ds-color-text-muted)" }}>
                    {g.setores.length.toLocaleString("pt-BR")}
                  </span>
                )}
                <span className="w-16 text-right shrink-0 font-semibold" style={{ color: "var(--ds-color-primary-600)" }}>
