...
def get_outlinks_flat(period: str, date: str, site_id: str | None = None, limit: int = 5000, timeout: int = 60) -> list:
    ...
    domínio). Usado pela rota `/api/analytics/cartas/acessos` (on-demand):
    1 chamada devolve todos outlinks do range e o server cruza com
    `urlExterno` do inventário. Sub-segundo."""
    return _call(
        "Actions.getOutlinks",
        period,
        date,
